...
""": b(
    ...
    a(s"vwx", b(p(s"yza", s"bcd"))),
    a(s"efg", b(p(s"hij", s"klm")))),
  """
abc::
  def ghi
  jkl mno ::
...
let bracketTests = {
  "a = [123, 456 789, abc = \"def\" ghi]": b(a(s"a", b(
    p(s"123"),
    p(s"456"),
    p(s"789"),
    a(s"abc", t"def"),
...
""": b(
    ...
    a(s"vwx", p(s"yza", s"bcd")),
    a(s"efg", p(s"hij", s"klm"))),
  """
abc |
  def ghi
  jkl mno |
...
""": b(
    ...
    a(s"vwx", b(p s"yza", p s"bcd")),
    a(s"efg", b(p s"hij", p s"klm"))),
  """
abc ||
  def ghi
  jkl mno ||
...
""": b(p(s"abc", a t"def", s"ghi", a p(s"jkl", s"mnop"))),
  "a = b = c": b(p(s"a", a s"b", a s"c")),
  # blocks:
  """
...
""": b(a(s"abc", t"def"), p(t"this is # not a comment")),
    """
abc: def ghi
abc:
  def ghi
...
vwx"""),
    ...
    a(s"a", t("b c\nd")),
    a(s"a", b(p(s"b", s"c"), p(s"d")))
  ),
  # brackets:
  """abc = [def, "ghi jkl" mnop
, (nested "phrase")]""": b(a(s"abc", b(p(s"def"), p(t"ghi jkl"), p(s"mnop"), p(p(s"nested", t"phrase"))))),
  # pipes:
  """abc | def, "ghi jkl" mnop
abc |
  def, "ghi jkl"
...
abc ||
  ...
  def |=
    ghi ||
      jkl
  mno: qrs
  tuv ::= wxyz""": b(p(s"abc", p(
    s"def", a p(s"ghi", b(p s"jkl")),
    s"mno", t"qrs",
    s"tuv", a b(p(s"wxyz")))))
}
